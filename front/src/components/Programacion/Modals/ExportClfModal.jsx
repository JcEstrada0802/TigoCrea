...
import { FaFileCode, FaPencilAlt } from 'react-icons/fa';
import axios from 'axios';

const ExportClfModal = ({ isVisible, onClose, onExport, position }) => {
    ...
    const handleSaveClick = () => {
        onExport(ClfName, selectedDate, selectedCalendar );
        setClfName(''); // Reset
        onClose();
    };
    ...
    return (
        ...
        >
            ...
            <div className="mb-5 flex items-start gap-3">
                ...
                <div>
                    <h2 className="text-xl font-bold text-[#001EB4]">Exportar playlist a CLF</h2>
                    <p className="text-xs text-gray-500 mt-1">Exporte la playlist del dia que seleccione a un archivo .clf</p>
                </div>
            </div>

            {/* INTRODUCIR NOMBRE DEL ARCHIVO.CLF */}
            <div className="space-y-4">
                <div>
                    <label className="flex items-center gap-2 text-[9px] font-black text-gray-400 mb-1.5 ml-1 tracking-tighter">
                        <FaPencilAlt className="text-blue-400" /> Nombre del archivo CLF
                    </label>
                    <input 
                        autoFocus
                        type="text" 
                        placeholder="Ej. PlaylistTS, PlayTS+, Lista..."
                        value={ClfName}
            ...
            <div>
                <label className="flex items-center gap-2 text-[9px] font-black text-gray-400 mb-1.5 ml-1 tracking-tighter">
                    <FaCalendarAlt className="text-blue-400" /> Fecha
                </label>
                <input 
                    type="date"
                    value={selectedDate} // Asegurate de tener este estado definido
                    onChange={(e) => setSelectedDate(e.target.value)}
                    className="w-full px-4 py-2.5 bg-gray-50 border-none rounded-2xl text-[11px] font-bold text-gray-600 focus:ring-2 focus:ring-blue-400 outline-none cursor-pointer appearance-none"
                />
            </div>
            ...
            <div className="mt-8">
                <button 
                    ...
                    disabled={!ClfName.trim() | !selectedCalendar.trim() | !selectedDate.trim()}
